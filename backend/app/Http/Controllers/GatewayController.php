...
class GatewayController extends Controller
{
    public function settings(Request $request)
    {
        ...
        return Inertia::render('Settings/PaymentGateway', [
            'config' => $config?->only(['id', 'enabled', 'qr_type', 'bank_account_id', 'provider']),
    ...
    public function configure(Request $request)
    {
        ...
        DB::transaction(function () use ($request, $data) {
            ...
            if ($config && GatewayTransaction::where('payment_gateway_config_id', $config->id)->where('status', 'pending')->where('expires_at', '>', now())->exists() && ($recipient !== $config->recipient_id || $data['bank_account_id'] !== $config->bank_account_id || $data['qr_type'] !== $config->qr_type)) {
                throw ValidationException::withMessages(['recipient_id' => 'Wait for active QR intents to expire before changing the recipient.']);
            }
            $config = PaymentGatewayConfig::updateOrCreate(['org_id' => $request->user()->org_id], ['provider' => 'settlement_hmac', 'enabled' => $data['enabled'], 'qr_type' => $data['qr_type'], 'recipient_id' => $recipient, 'webhook_secret' => $secret, 'bank_account_id' => $data['bank_account_id']]);
    ...
    public function qr(Request $request, Invoice $invoice, GatewaySettlementService $gateway, PortalAccessService $portal)
    {
        ...
        return response()->view('documents.payment-qr', ['transaction' => $tx, 'image' => app(PromptPayQrService::class)->image($tx->qr_payload), 'invoice' => $invoice])->header('Cache-Control', 'private, no-store');
    }
